...
//----------------------------------------------------------------------------
// This function is used to detect which platform Hipass is running on, so in
// this way it can be platform agnostic between Windows, MacOS and Linux. This
...
void copy_to_clipboard_prompt(const char *str)
{
    ...
    do
    {
        printf(C_WHITE "Do you want to copy the password to the clipboard? "
                       "(y/n): " C_RESET);
        clipPrompt = getchar();
